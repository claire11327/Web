...
$(".dropdown-toggle").parent().on("click",function(){//é¦–é€‰å•ç‚¹å‡»ä¸è·³è½¬
if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
if(flag==1)
...
var count = moCount||1;
var moContent = [];
if(first){//ç¬¬ä¸€ä¸ªåŠ è½½çš„é¡µé¢åˆå§‹åŒ–ç€‘å¸ƒæµ
...
var appendFunc = function(str, moDivId){//ç”¨äºŽå¾€ç€‘å¸ƒæµæ·»åŠ å…ƒç´ 
var moStr = str||'';
var itemStr = '';
if(!first && moStr){
...
if(module.length>0 && module.html().trim().length>0){//èŽ·å–å†…å®¹ä¸ä¸ºç©ºçš„æ¨¡ç»„,åŠ å…¥ç€‘å¸ƒæµ
...
functionÂ fixMenuPosition2(par,Â id,Â cnt,Â offset,Â flag)Â {
Â Â Â Â Â Â Â Â varÂ pÂ =Â par;
Â Â Â Â Â Â Â Â varÂ scrollTopÂ =Â document.body.scrollTopÂ +Â document.documentElement.scrollTop;
Â Â Â Â Â Â Â Â varÂ clientHeightÂ =Â document.documentElement.clientHeight;
Â Â Â Â Â Â Â Â varÂ clientWidthÂ =Â document.documentElement.clientWidth;
Â Â Â Â Â Â Â Â varÂ objÂ =Â document.getElementById(id);
Â Â Â Â Â Â Â Â obj.parentNode.classList.add('open')
Â Â Â Â Â Â Â Â offsetTopÂ =Â 0;
Â Â Â Â Â Â Â Â whileÂ (parÂ !==Â null)Â {
Â Â Â Â Â Â Â Â Â Â Â Â offsetTopÂ +=Â par.offsetTop;
Â Â Â Â Â Â Â Â Â Â Â Â parÂ =Â par.offsetParent;
Â Â Â Â Â Â Â Â }
Â Â Â Â Â Â Â Â 
Â Â Â Â Â Â Â Â ifÂ (typeofÂ offsetÂ ==Â "undefined")Â offsetÂ =Â 10;
Â Â Â Â Â Â Â Â offsetÂ =Â 5;
Â Â Â Â Â Â Â Â varÂ chÂ =Â obj.offsetHeight;
Â Â Â Â Â Â Â Â varÂ diffÂ =Â (offsetTopÂ +Â chÂ +Â offset)Â -Â (scrollTopÂ +Â clientHeight);
Â Â Â Â Â Â Â Â 
Â Â Â Â Â Â Â Â varÂ diff_wÂ =Â obj.parentNode.offsetLeftÂ +Â obj.parentNode.offsetWidthÂ +Â obj.offsetWidth;
Â Â Â Â Â Â Â Â ifÂ (diffÂ >Â 0)Â {
Â Â Â Â Â Â Â Â Â Â Â Â obj.style.topÂ =Â "-"Â +Â diffÂ +Â "px";
Â Â Â Â Â Â Â Â }Â elseÂ {
Â Â Â Â Â Â Â Â Â Â Â Â obj.style.topÂ =Â "-1px";
Â Â Â Â Â Â Â Â }
Â Â Â Â Â Â Â Â ifÂ (diff_wÂ >Â clientWidth)Â {
Â Â Â Â Â Â Â Â Â Â Â Â obj.style.leftÂ =Â "auto";
Â Â Â Â Â Â Â Â Â Â Â Â obj.style.rightÂ =Â "100%";
Â Â Â Â Â Â Â Â }Â elseÂ {
Â Â Â Â Â Â Â Â Â Â Â Â obj.style.leftÂ =Â "100%";
Â Â Â Â Â Â Â Â }
Â Â Â Â }
...
function tabCarousel(p_owl,p_id,p_left,p_right,p_dot,p_type){
$(p_id+" div.cloned a").attr("tabIndex","-1");
$(p_id+" button.owl-dot").attr("tabIndex","-1");
if(p_type=='gallery'){
$(".owl-thumb .owl-thumbitem a").attr("tabIndex","-1");
}
